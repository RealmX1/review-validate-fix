...
``SpawnRecord`` 描述「一次子代理派生」的足够元数据，以便定位其独立 transcript
并写进 manifest。它对 host 中性：Codex 的 ``spawn_agent`` 与 Claude 的
``Task``/``Agent`` 都归一到同一结构，host 特定的「如何发现 / 如何定位 rollout」
逻辑放 ``adapters/<host>/subagent.py``。
