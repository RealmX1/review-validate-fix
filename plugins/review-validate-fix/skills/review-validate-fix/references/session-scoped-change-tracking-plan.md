...
- 当 manifest 含 `tracker.tracker_scope`（Slice 2-B+），packet 用 `## Allocated Git Diff` 替代 `## Session-Owned Git Diff`，并在 `## Session Manifest` 后插入 `## Tracker Scope` audit section（unit_ids / lease_id / scope_hash / paths / hunks）。
