...
5. `CONTRACT_VIOLATION` 的输出不得进入合格 merge；可归一化小格式漂移不记为 `CONTRACT_VIOLATION`，可在最终汇总中作为低风险注记一笔带过。`WORKSPACE_CHANGED_DURING_REVIEW` 需要主会话先检查污染范围；如果只是可解释测试副作用，仍可保留 reviewer 输出；如果出现未授权源文件、lockfile、snapshot 或文档写入，应 fail-close 或询问用户。
