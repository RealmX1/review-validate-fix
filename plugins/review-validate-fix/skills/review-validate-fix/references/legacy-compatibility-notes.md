...
旧 `stop-review-validate-fix.sh` 的 handoff/time-travel 设计假设：Stop hook 触发后，用户可以从“hook 触发之前”的任意会话位置 fork 或回退，然后把结构化 handoff 内容贴给 earlier-self。
...
- 可以保留 `handoff.md` 作为人工可读、可打开的上下文压缩文件，用于用户在可回退的输入边界或新会话中继续工作。
...
新 fork 会话完成 `$review-validate-fix` 后，如果 Stop 事件的 `last_assistant_message` 或 transcript 末尾包含 `RVF_HANDOFF_FILE: <handoff.md 绝对路径>`，Stop hook 会在 dev sync gate 和 dirty/fork gate 前把该文件作为完成信号处理，并默认自动打开 handoff markdown 文件。agent 正文只输出文件路径，不重复 handoff 内容。
...
- 正式 review fork 不设置 `CODEX_RVF_SUPPRESS_STOP_HOOK=1`，否则结束时无法自动处理 `RVF_HANDOFF_FILE`；实验 fork 可以设置该 suppress 标记。
