...
唯一例外（必须打破「只给两行标签摘要」）：本轮有任何 `[elevated]` 条目时，最终回复必须在两行标签摘要之外，当场把每条升级直接摆给用户——逐条简述「卡在哪 + 候选方向」，不得只埋进 `handoff.md` 正文。升级 = 需用户决策；其中属「只有用户能拍板的方案选择（trade-off）」的，用 `AskUserQuestion` 把候选选项摆出来让用户当场选，而不是写在散文里等下一轮。
