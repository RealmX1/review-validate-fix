...
Handoff 默认开启，但只适用于 `mode: full` 且 handoff 未关闭的完整流程。除非用户明确要求 `no handoff` / `skip handoff` / `不要 handoff`，主会话必须在当前 RVF run 的 artifacts 目录创建并持续维护 `handoff.md`，最终回复第一行输出 handoff 路径，空一行后按下面的固定结构分两行追加极短中文摘要（每行各自一句、不要挤成一段）：
...
不要在最终回复里重复 handoff 文件正文；摘要只服务快速确认，完整细节必须在 `handoff.md`。Stop hook 检测到 `RVF_HANDOFF_FILE` 后作为完成信号处理，run 结束时通过 terminal-notifier 发送一条 OS 系统通知（kanban 来源的通知点击后可打开对应 task）；不再自动用编辑器打开 handoff，也不要手动调用任何「打开 handoff」脚本。

唯一例外（必须打破「只给两行标签摘要」）：本轮有任何 `[elevated]` 条目时，**在所有非 elevated issue 验证/修复完、handoff 与相关 brief 更新完之后**，最终回复必须在两行标签摘要之外、于 workflow 末端逐条把升级直接摆给用户——简述「卡在哪 + 候选方向」，不得只埋进 `handoff.md` 正文，也不在 review 一发现就当场打断流程去问（应先由主会话自解析：自身 context → 派 subagent 找决策依据，二者都不足才记为 elevated）。升级 = 需用户决策；其中属「只有用户能拍板的方案选择（trade-off）」的，用 `AskUserQuestion` 把候选选项摆出来让用户当场选。
...
- 最终阶段更新 repo delta、验证命令、继续指引、升级事项和 deterministic intake hints；最终回复只给文件路径和分行的 `Reviewers：` / `Validate/fixers：` 标签摘要。
