...
本目录是 Review-Validate-Fix 内部使用的定制 review standards pack。它提炼自 `agent-skills` 的 Review 类 skills，但已按 RVF 的 session-scoped scope、strict artifact contract、validate/fix 和 handoff 约束改写。
...
- `protocol-extensions.md`：review result artifact 中的 request 非完成态协议。
