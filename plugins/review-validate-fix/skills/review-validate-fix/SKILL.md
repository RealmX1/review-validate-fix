...
例外：如果用户已经配置本 skill 附带的 Codex Stop hook，该 hook 可以在明确的 dirty repo 停止点通过 Codex app-server `thread/fork` + `turn/start` 创建一个新的 GUI fork，或在 Cline Kanban 当前 task 中通过 host 定制的真实 follow-up user message 启动 review loop。这属于用户预配置脚本生成的显式 prompt 边界，不是模型隐式启用 skill，也不改变 `allow_implicit_invocation: false` 的约束。
...
- Stop hook 一旦决定启动 RVF backend，必须在真正创建 GUI fork、Kanban task 或 follow-up 消息前执行 backend 对应的 provider health guard。Codex provider 必须先通过 `codex login status`，失败或显示过期时 fail-close 并提示用户运行 `codex login`；可用 `CODEX_RVF_PROVIDER_HEALTH_CHECK=0` 临时跳过，用 `CODEX_RVF_AUTO_CODEX_LOGIN=1` 在失败时尝试后台触发 `codex login`。
