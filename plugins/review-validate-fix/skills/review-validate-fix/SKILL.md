...
- dispatcher、Stop hook、manual run 和 external reviewer 使用统一 run ledger：`state/latest.json` 只作为 pointer，真实排障入口是 `state/runs/<run_id>/summary.json` 和 `events.jsonl`；prompt、review packet、stdout/stderr 等大文本必须作为 `artifacts/` 文件引用，不写入 hook stdout。可用 `CODEX_RVF_LOG_ROOT` 覆盖日志根目录，`CODEX_RVF_STATE_DIR` 只作为兼容别名；这些日志只服务排障，不是 review scope 证据本身。
