...
Codex CLI 兼容入口：当前 CLI 会把带 `allow_implicit_invocation: false` 的 standalone skill 从 model-visible skill 列表中隐藏，因此安装器会在 `~/.codex/config.toml` 启用 `review-validate-fix@local-codex-plugins`，并在 `~/.codex/skills/review-validate-fix` 生成一个轻量 launcher。该 launcher 只在用户消息精确包含 `$review-validate-fix` 或明确按 exact name 调用 plugin 时触发，并指向 installed plugin runtime 的本文件；不要把 runtime skill 原样复制为 CLI compatibility entry。
