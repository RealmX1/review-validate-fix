...
7. `prepare_review_run.py` 输出中的 `review_env_file` / `review_env` 是 subprocess 可直接使用的短期路径上下文；`review_agent_context_file` / `review_agent_context` 是给 Codex-native reviewer 子代理的程序化入口块。后续 prompt、命令示例和子代理交接应复用这个生成块，或引用其中的 `RVF_RUN_DIR`、`RVF_ARTIFACTS_DIR`、`RVF_SCOPE_OF_WORK`、`RVF_SESSION_MANIFEST`、`RVF_REVIEW_PACKET`、`RVF_COMMAND_LOCK` 等变量；不要由主会话手写 export block，也不要在同一段 prompt 中反复展开 `state/runs/<run_id>/artifacts/...` 的绝对路径。
...
- 显式 `CODEX_RVF_FORK_MODE=vibe-kanban`：dirty gate 通过后，脚本不调用 Codex GUI fork；默认 `CODEX_RVF_VK_MANAGEMENT_MODE=local-workspace`。hook 必须先确保 Vibe-Kanban app/backend 可用：优先连接现有 `CODEX_RVF_VK_MCP_CMD`，MCP client 必须使用 Vibe-Kanban 0.1.44 的 JSON-lines stdio；backend 优先级为 `CODEX_RVF_VK_BACKEND_URL`/`VIBE_BACKEND_URL`、`vibe-kanban.port`、本机 Vibe-Kanban 监听端口试探，且发现到的本地 backend 要优先于 MCP 默认自动发现以便日志记录实际 app server。不可用时用 `tmux` session `rvf-vibe-kanban` 启动 `CODEX_RVF_VK_START_CMD`（默认 `npx -y vibe-kanban@0.1.44`，默认等待 `CODEX_RVF_VK_START_TIMEOUT=90`）。默认路径通过本地 `/api/workspaces` 创建 RVF 管理 workspace，名称反映 `queued/running/completed/failed/cancelled`，详情写入该 workspace 的 `WORKSPACE_NOTES` scratch；不得调用 Vibe 的 `start_workspace`，不得创建 Vibe 管理的独立 worktree。显式 `CODEX_RVF_VK_MANAGEMENT_MODE=remote-project` 时才使用旧 project/issue 路径：installer 可通过 `--vibe-kanban-project-id` 或 `CODEX_RVF_VK_PROJECT_ID` 固定 project id，未提供时写入 `CODEX_RVF_VK_PROJECT_AUTO=1`，再通过 `list_repos`/`list_projects` 匹配当前 repo；如果当前 MCP 暴露 `create_project`，无匹配时可创建项目；如果不能创建但只有一个 project，可用 `single_project_fallback`，并允许 `CODEX_RVF_VK_SINGLE_PROJECT_FALLBACK=0` 禁用。管理记录创建成功后启动 `scripts/run_vibe_kanban_rvf.py` 调用 `CODEX_RVF_CODEX_EXEC_ARGS`（默认 `exec --json --dangerously-bypass-approvals-and-sandbox`）执行 `codex exec -C <target_repo>`。runner 传给 `codex exec` 的 prompt 必须包装原始 GUI fork prompt，并注入 parent transcript path、target repo、run id/run dir、existing artifacts 和 `prepare_review_run.py --rvf-run-id --rvf-run-dir` 复用要求；headless agent 不得假设拥有父 GUI 会话的内存上下文。该模式仍复用同一个 RunLedger 和同一 worktree；Vibe-Kanban 只作为管理平面。Vibe-Kanban app/backend 不可用、无法创建本地 workspace，或显式 `remote-project` 模式下无法匹配 project 时必须 fail-safe，不启动隐藏 runner。
- 停止正在运行的 Vibe-Kanban/headless RVF run 时，读取 `references/cancel-rvf-run.md` 并使用 `scripts/cancel_rvf_run.py`；用户主动停止必须标为 `cancelled`，不得把 `SIGTERM` 或 negative return code 记为 `failed`。
- self-hosted Vibe-Kanban Cloud 需要把本地 Cloud 连接参数持久写入 Stop hook。installer 必须支持并 dispatcher 自同步必须保留 `--vibe-kanban-mcp-cmd`、`--vibe-kanban-start-cmd`、`--vibe-kanban-backend-url`，对应 `CODEX_RVF_VK_MCP_CMD`、`CODEX_RVF_VK_START_CMD`、`CODEX_RVF_VK_BACKEND_URL`；例如 start/mcp command 可包含 `env VK_SHARED_API_BASE=http://localhost:3000 ...`。`CODEX_RVF_VK_BACKEND_URL` 只能指向本地 Vibe app backend（`http://127.0.0.1:<port>`），不能指向 Cloud remote-server；默认 local-workspace 路径通过本地 app 的 `/api/workspaces` 管理 RVF 记录。不要假设 `npx vibe-kanban` 本身就是完整本地 Cloud issue/project 后端。
