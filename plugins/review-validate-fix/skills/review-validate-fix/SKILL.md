...
- `mode: rvf_analyze` / `$rvf-analyze`：事后复盘已 finalize 的 RVF run，**不**启动新 review、不改源码、不生成 handoff.md。入口脚本是 `scripts/rvf_analyze.py`，详细操作手册见 `references/rvf-analyze.md`。脚本退出码 2/3 表示需要询问用户（``running`` / ``orphan_candidate`` / ``cancel_without_lock``），决策后用 ``--force`` / ``--auto-finalize-orphan`` / ``--decline-finalize`` 重入。复盘只写 ``<run_dir>/artifacts/analysis/`` 与 ``.interrupted`` marker，不修改其他 artifact。
