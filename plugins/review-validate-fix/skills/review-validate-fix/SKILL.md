...
- 最终回复第一行输出 `RVF_HANDOFF_FILE: <绝对路径>`，空一行后按固定结构分两行追加极短中文摘要：`Reviewers：…` 一行、`Validate/fixers：…` 一行（结构见 `references/handoff-template.md`），每行各自一句、不要挤成一段。Stop hook 会把该 marker 当作完成信号，run 结束时发送 OS 系统通知（不再自动用编辑器打开 handoff）；不要再手动调用任何「打开 handoff」脚本。
