...
- installed Stop hook 入口为 plugin skill 内的 `scripts/codex_stop_hook_router.py`，它只按 `RVF_STOP_HOOK_CHANNEL` 控制行或 session marker 选择 stable/dev channel；默认永远是 stable，不得因为当前 repo 是 RVF 源仓库或匹配 `CODEX_RVF_DEV_REPO` 就自动切 dev。router 选中 channel 后转发到对应 dispatcher，再由 dispatcher 把同一份 Stop event JSON 转交给 `scripts/codex_stop_review_validate_fix.py`。真实 fork gate 逻辑仍只放在 `codex_stop_review_validate_fix.py`；不要在 hook 脚本里直接执行 review/fix。
...
- dispatcher 同步只允许在直接作为开发入口运行、Stop event 的 git root 等于 `CODEX_RVF_DEV_REPO`、事件不是 subagent，且 session manifest 显示存在当前 session-owned dirty paths 时运行；经 router 运行的 stable/dev channel 必须关闭 repo-path dev sync。同步失败必须输出不会触发模型续跑的 hook payload，并给出 `summary.json` 路径，避免继续使用 stale installed plugin skill。不要用非零 stderr 表达这类失败，因为 Codex Desktop 可能把 stderr 包成当前会话的 `<hook_prompt>` continuation。
