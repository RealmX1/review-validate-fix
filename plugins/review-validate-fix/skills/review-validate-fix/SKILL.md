...
- 如果 Desktop control socket 不可用，默认不要启动独立 bridge app-server fork，因为 bridge 创建出的 session 可能不会被当前 Desktop GUI 立即订阅和显示；此时自动回退为 Stop continuation，在当前会话中可见地运行 `$review-validate-fix`。只有显式设置 `CODEX_RVF_BRIDGE_GUI_UNVERIFIED_POLICY=bridge` 或 `CODEX_RVF_ALLOW_BRIDGE_APP_SERVER=1` 时，才允许使用 bridge fallback。
- `CODEX_RVF_MODE=continuation` 是显式 fallback：脚本会返回 Codex Stop continuation hook 的 `decision: "block"` / continuation prompt，在当前会话中继续运行 `$review-validate-fix`。该模式不会产生独立 fork checkpoint。
