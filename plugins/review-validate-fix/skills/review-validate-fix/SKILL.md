...
- **`$rvf-analyze` 是独立的 sister skill**（`plugins/review-validate-fix/skills/rvf-analyze/`），用于事后复盘已 finalize 的 RVF run。本 skill 不再承担该 mode；不要因为上下文出现 `$rvf-analyze` 字样就把它当作本 skill 的窄模式处理。
