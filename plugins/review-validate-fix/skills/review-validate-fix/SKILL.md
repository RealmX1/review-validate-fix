...
- dispatcher 同步只允许在 Stop event 的 git root 等于 `CODEX_RVF_DEV_REPO` 且事件不是 subagent 时运行；同步失败必须以非零 stderr 阻止 fork gate，并给出 `summary.json` 路径，避免继续使用 stale installed plugin skill。
