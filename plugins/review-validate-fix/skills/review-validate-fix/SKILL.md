...
例外：如果用户已经配置本 skill 附带的 Codex Stop hook，该 hook 可以在明确的 dirty repo 停止点创建新的 Cline Kanban task，或在 Cline Kanban 当前 task 中通过 host 定制的真实 follow-up user message 启动 review loop。Codex GUI/app-server `thread/fork` + `turn/start` 只保留为显式 `CODEX_RVF_FORK_MODE=gui` 或显式 legacy opt-in 路径，不是默认路径。这属于用户预配置脚本生成的显式 prompt 边界，不是模型隐式启用 skill，也不改变 `allow_implicit_invocation: false` 的约束。
...
5. 如果 Stop event、fork prompt 或当前环境提供 Codex JSONL transcript path，先用 `scripts/session_manifest.py --repo <repo> --transcript <jsonl> --output <manifest>` 生成 session ownership manifest。manifest 中的 `owned_paths` / `owned_dirty_paths` 是生成 tracker units、bootstrap 和 scope contract 的输入证据；冻结后 reviewer 不再直接把 manifest 当作最终 scope contract。`unattributed_dirty_paths` 是背景 WIP，不得主动审查，除非它被 `scope.contract.json` 范围内改动直接连带影响。
6. 如果当前 fork prompt 或环境已经提供预冻结 artifacts，例如 `RVF_RUN_DIR` / `RVF_ARTIFACTS_DIR` / `RVF_REVIEW_ENV` / `RVF_REVIEW_PACKET` / `RVF_SCOPE_CONTRACT`，先 source 既有 `review-env.sh` 并复用这些文件；不得重新运行 `prepare_review_run.py` 创建新的 run，尤其不得把 run 写入 Cline Kanban task 的 `.cline/worktrees/...` worktree。否则，用 `scripts/prepare_review_run.py --repo <repo> --session-context <file> --transcript <jsonl>` 或 `--session-manifest <manifest>` 创建唯一 run 目录；该脚本会把 scope-of-work 和 manifest 复制到 run 目录的 `artifacts/inputs/`，并生成 self-contained review packet、packet metadata、review 前 workspace snapshot、不可变 `scope.contract.json`、`review-env.sh` 和 `review-agent-context.md`。需要手动生成 packet 时，可用 `scripts/build_review_packet.py --repo <repo> --session-context <file> --session-manifest <manifest> --output <packet> --metadata-output <metadata>`。reviewer 启动后必须先读取 `$RVF_SCOPE_CONTRACT`：`primary_units` 非空时按 unit scope 审查；否则按 `primary_files` / scope-of-work 的 path scope 审查。
...
- **`$rvf-analyze` 是独立的 sister skill**（`plugins/review-validate-fix/skills/rvf-analyze/`），用于事后复盘已 finalize 的 RVF run。本 skill 不再承担该 mode；不要因为上下文出现 `$rvf-analyze` 字样就把它当作本 skill 的窄模式处理。RVF finish/finalize 会自动生成 `artifacts/analysis/summary.md` 与 `artifacts/analysis/causality.json` 的确定性 scaffold；显式 `$rvf-analyze` 仍只负责后续 LLM 叙事补全与 issue ↔ patch 因果归属。
...
- 核心设计支柱：Stop hook 自动化必须让触发 hook 的父会话在 hook 完成后停止，并默认在新的 Cline Kanban task 或当前 Kanban task follow-up 中留下 review checkpoint。Kanban 不可用时默认报告诊断态并停止，不自动退回 Codex GUI fork。Codex GUI fork 只允许通过显式 `CODEX_RVF_FORK_MODE=gui` 或显式 legacy opt-in 作为 backup-of-backup，保留父会话完整上下文并由 hook 注入以 `$review-validate-fix` 开头的首个用户 prompt；不要把当前会话 continuation 当作自动路径。
- installed Stop hook 入口为 plugin skill 内的 `scripts/codex_stop_hook_router.py`，它只按 `RVF_STOP_HOOK_CHANNEL` 控制行或 session marker 选择 stable/dev channel；默认永远是 stable，不得因为当前 repo 是 RVF 源仓库或匹配 `CODEX_RVF_DEV_REPO` 就自动切 dev。router 选中 channel 后转发到对应 dispatcher，再由 dispatcher 把同一份 Stop event JSON 转交给 `scripts/codex_stop_review_validate_fix.py`。stable channel 必须关闭子 dispatcher 的 repo-path dev sync；dev channel 可以启用 repo-path dev sync 做 contract/preflight，但必须设置 `CODEX_RVF_DEV_SYNC_INSTALL=0`，不得通过 Stop hook 自动更新 stable plugin。真实 fork gate 逻辑仍只放在 `codex_stop_review_validate_fix.py`；不要在 hook 脚本里直接执行 review/fix。
...
- Stop hook 一旦决定启动 RVF backend，必须在真正创建 Kanban task、follow-up 消息、显式 GUI fork 或显式 legacy GUI fallback 前执行 backend 对应的 provider health guard。Codex provider 必须先通过 `codex login status`，失败或显示过期时 fail-close 并提示用户运行 `codex login`；可用 `CODEX_RVF_PROVIDER_HEALTH_CHECK=0` 临时跳过，用 `CODEX_RVF_AUTO_CODEX_LOGIN=1` 在失败时尝试后台触发 `codex login`。
- 如果受控开发路径直接启用 dispatcher dev sync，且 Stop event 提供 transcript path，dispatcher 必须先生成 session manifest；只有当前 chat session 存在 `owned_dirty_paths` 时，才执行 dev sync、安装和 installed hook 转交。若 dirty repo 只有 `unattributed_dirty_paths` 或没有 session-owned dirty paths，dispatcher 应输出跳过 payload 并停止自动 review，避免把其他 session / agent 的 WIP 归到当前 session。
- dispatcher 同步只允许在直接作为开发入口运行、Stop event 的 git root 等于 `CODEX_RVF_DEV_REPO`、事件不是 subagent，且 session manifest 显示存在当前 session-owned dirty paths 时运行；经 router 运行时，stable channel 必须关闭 repo-path dev sync，dev channel 只允许 repo-path contract/preflight sync，不能运行 install step。同步失败必须输出不会触发模型续跑的 hook payload，并给出 `summary.json` 路径，避免继续使用 stale installed plugin skill。不要用非零 stderr 表达这类失败，因为 Codex Desktop 可能把 stderr 包成当前会话的 `<hook_prompt>` continuation。
- dev-only sync chain 必须只从 `CODEX_RVF_DEV_REPO` 解析并运行仓库级 `scripts/check_plugin_contracts.py` 与 `scripts/install_to_codex.py`；这些脚本和未来 dev-only helper 不得部署进 plugin runtime/cache。router dev channel 只能运行仓库级 `scripts/check_plugin_contracts.py`，stable channel 更新必须由人工在目标 stable checkout/tag 上显式运行 `python3 scripts/check_plugin_contracts.py` 与 `python3 scripts/install_to_codex.py --configure-stop-hook`。runtime 代码若需要开发期能力，只能经 `CODEX_RVF_DEV_REPO` subprocess 跨边界调用，不能 import 或复制 dev-only 模块。
...
- 排查 `no_session_owned_dirty`、manifest 未写入 tracker、Claude/Codex transcript 归属缺口或 installed/runtime 版本漂移时，优先运行 `scripts/diagnose_stop_hook_scope.py --summary <summary.json>`。该脚本只读 summary/artifacts/transcript 和 runtime 脚本指纹，不修改 tracker state。
- 默认 `CODEX_RVF_MODE=fork` 且 `CODEX_RVF_FORK_MODE=auto`：dirty gate 通过后，脚本会从 Stop event / 环境自动发现 backend；当前运行在 Kanban task 内并提供 `KANBAN_TASK_ID` / `CLINE_KANBAN_TASK_ID` / `KANBAN_HOOK_TASK_ID` / `task_id` 时走 `kanban-followup`，否则创建新的 Cline Kanban task。Cline Kanban task 启动失败时默认只报告 `cline-kanban-unavailable` / `cline-kanban-unconfigured` 诊断和 summary，不再自动创建 Codex GUI fork。Codex GUI fork 只能由用户显式配置 `CODEX_RVF_FORK_MODE=gui`，或用 `CODEX_RVF_AUTO_LEGACY_GUI_FALLBACK=1` 显式恢复 legacy backup-of-backup；legacy fallback 使用时 summary 必须标记 `legacy_gui_fallback` 和 `legacy_gui_fallback_enabled=true`。
- 显式 `CODEX_RVF_FORK_MODE=cline-kanban`（别名 `cline` / `kanban` / `ck`）：dirty gate 通过后，脚本不调用 Codex GUI fork，也不后台启动隐藏 `codex exec`。hook 必须先运行 `prepare_review_run.py` 生成 review packet、session manifest、workspace snapshot，并额外冻结 worktree bootstrap artifact：`worktree-bootstrap.patch`、`worktree-bootstrap-files/`、`worktree-bootstrap.json`；这些 bootstrap artifact 只允许包含当前 session-owned dirty paths，遇到无法安全重放的情况必须 fail-close，不启动 Kanban task。hook 随后用 `scripts/cline_kanban_client.py` 调用官方 `kanban` CLI：先 `kanban task list --project-path <repo>` 检查服务可用，失败时通过 tmux 启动 `CODEX_RVF_CLINE_KANBAN_START_CMD`（默认 `kanban --no-open`，session 默认 `cline-kanban-3484`，等待默认 `90` 秒），再 `kanban task create --project-path <repo> --base-ref <base> --prompt <generated-prompt>` 和 `kanban task start --project-path <repo> --task-id <id>`。默认路径依赖 PATH 上稳定安装的 `kanban` binary；旧的精确默认值 `npx -y kanban@0.1.66 ...` 只作为 legacy default 被安装器/dev-sync 迁移掉，不应作为 Stop hook 长期配置。复用既有 listener 时，listener pane 必须属于 tmux `cline-kanban` 或 `cline-kanban-*` session；不再用 listener cwd 判断目标 repo 归属。RunLedger summary 必须写入 `cline_kanban_task_id`、`workspace_path`、`cline_kanban_base_ref`、`worktree_bootstrap_path`、task prompt artifact、`rvf_dispatch_target_worktree` 和 `rvf_dispatch_target_kanban_task_id`。Kanban task 创建后必须用 CLI 返回的 `workspace_path` 回填 prep file 的 `target_worktree`；hook payload 必须提示用户在 Flow 2 branch mode 下暂停 origin worktree 编辑，等待 `RVF_HANDOFF_FILE` 返回后再合并。Kanban task prompt 必须先在 task worktree 内解析 repo root，设置 `RVF_RUN_DIR` / `RVF_ARTIFACTS_DIR` / `CODEX_RVF_LOG_ROOT` / `CODEX_RVF_RUN_ID` / `CODEX_RVF_RUN_DIR`，source `$RVF_ARTIFACTS_DIR/review-env.sh`，再把 `RVF_REPO` 覆盖为 task worktree root，并用 `$RVF_WORKTREE_BOOTSTRAP` 调用 `scripts/apply_worktree_bootstrap.py`。后续说明必须继续复用 `$RVF_REVIEW_PACKET`、`$RVF_SESSION_MANIFEST`、`$RVF_WORKTREE_BOOTSTRAP`、`$RVF_ARTIFACTS_DIR/handoff.md` 等变量，执行完整 `$review-validate-fix` 并维护 handoff；不要在同一个 Kanban prompt 里反复展开 `state/runs/<run_id>/artifacts/...` 的绝对路径，也不要在 task worktree 里新建 `prepare-run` run。
...
- 手动 `$review-validate-fix`、`kanban-followup` 和新建 Cline Kanban task 必须共享同一套 `rvf_state` summary 字段：backend 归一为 `manual` / `kanban-followup` / `kanban-task`，phase 使用 `prepare -> review -> merge -> validate_fix -> verify -> handoff -> complete`，scope 来源固定为 prepare 阶段冻结的 `scope.contract.json`、scope-of-work 和 review packet；session manifest 是证据和 tracker splice 容器，不是后续 reviewer 的最终 scope contract。不得用后续 live diff 重新定义 scope。
...
- 如果显式 GUI fork 或显式 legacy GUI fallback 需要使用 app-server，且 Desktop control socket 不可用，默认 `CODEX_RVF_BRIDGE_GUI_UNVERIFIED_POLICY=auto` 会优先复用可连通的 RVF bridge app-server，必要时尝试启动 bridge app-server，并在 summary 中标记 `socket_source=bridge` / `gui_visibility=unverified-bridge-only`。如果复用 bridge 后 fork/turn 返回配置加载或权限类 app-server 错误，hook 必须只终止 RVF 专用 socket 上的旧 bridge listener、重启 bridge 并重试一次。不要回退为 Stop continuation prompt：它不会创建真正的新用户 prompt，只会作为 hook system context 出现在当前轨迹中，容易误导主会话重新运行流程。需要禁止 bridge fallback 时，显式设置 `CODEX_RVF_BRIDGE_GUI_UNVERIFIED_POLICY=report|manual|fail`。
...
- 当前 chat session 可用显式用户消息行管理 hook：`RVF_STOP_HOOK: off` 会把本 session 标记为 disabled，后续 Stop hook 对同一 session 只跳过 RVF fork/review gate；它不阻止 Stop hook router 运行或转发到当前选中的 stable/dev channel。`RVF_STOP_HOOK: on` 会清除该 session 标记并恢复，且本次 Stop event 会继续走正常 dirty/session scope/backend gate；如果本 session 仍有 session-owned dirty scope，它可以在同一轮创建 GUI fork 或 Cline Kanban task。`RVF_STOP_HOOK: status` 只报告当前 session gate 状态。当前 chat session 也可用 `RVF_STOP_HOOK_CHANNEL: dev` / `stable` / `default` / `status` 管理 stable/dev 安装选择；默认永远是 `stable`，router 不得因为当前 repo 是 RVF 开发仓库或匹配 `CODEX_RVF_DEV_REPO` 就自动切到 dev。`dev` / `stable` 会写入 session channel marker 并让当前 Stop event 继续走对应 channel；`default` 清除 marker；`status` 只报告当前 session 的 gate/channel 状态。这些 session 状态写入 `state/session-hook/`，只影响当前 chat session 的 RVF 执行 gate 和 channel，不修改全局 `~/.codex/hooks.json`。这些 `RVF_STOP_HOOK:*` 与 `RVF_STOP_HOOK_CHANNEL:*` 行只属于 Stop hook 会话控制元数据；除非用户明确申明其诉求与这些控制行本身的分析、更新有关，主会话或 fork 会话可以将其忽略。
- 手动 `$review-validate-fix` 完成后，主会话应在同一个 `state/session-hook/<session-id>.json` 里写入 `manual_rvf_completed_at` 和 `manual_rvf_run_id`。后续 Stop hook 仍允许 router 和 selected dispatcher 运行，但 installed hook 看到同 session 的 marker 后必须只跳过 RVF 主 workflow，并返回 `manual_rvf_already_ran`；不要把这个 gate 混同为 `CODEX_RVF_SUPPRESS_STOP_HOOK`，后者仍表示跳过整个 hook。
...
- Claude Code 主会话中需要派生 Codex alternative reviewer 时，优先使用 `config/alternative-reviewer.codex.json` 或同等配置调用 `scripts/run_alternative_reviewer.py --config <codex-config> ...`。该配置通过 `codex --ask-for-approval never exec --json --ephemeral --sandbox workspace-write -` 读取 prompt，健康检查为 `codex login status`，runner 以 `output_format: codex_json` 解析最终 reviewer 文本并仍以 `$RVF_REVIEW_RESULT` artifact 作为唯一 canonical result。若 Codex CLI 返回 ChatGPT/Codex backend challenge HTML 而不是 JSONL，runner 必须输出 `RVF_CODEX_BACKEND_CHALLENGE`、标记 `reviewer_codex_backend_challenge`，并把该 external reviewer 视为本轮无法启动。Codex CLI reviewer 需要 workspace-write 是因为 RVF review protocol 必须写 `$RVF_REVIEW_RESULT` artifact；review prompt 和 scope contract 仍禁止源码写回、stage、commit 或 validate/fix。
...
- 两个 reviewer 使用同一份 review prompt、同一个 scope-of-work 文件路径、同一份 `scope.contract.json`、同一份 session manifest 文件路径（如果有）和同一份 review packet 路径，但彼此不看对方输出。主会话不要把同一大段 scope 文本分别粘贴给两个 reviewer；把文件路径或本轮 `RVF_*` 变量交给它们读取即可，减少 prompt 重复。scope-of-work / session context 是主会话对本 turn 已完成工作的交接说明；`scope.contract.json` 是机器可读范围合同；session manifest 是机器提取的 ownership evidence。reviewer 应先按合同中的 `primary_units` / `primary_files` 确认 scope，再用 packet、diff、status、文件读取和验证命令独立核实。reviewer 的默认假设是：另一个独立 reviewer 可能正并行工作；因此命令需按锁规则协调。不得把整个 `git diff HEAD` 当作 full-scope analysis 来源；除非主会话明确要求 full diff review，否则只审查合同范围内改动及其直接连带影响。
