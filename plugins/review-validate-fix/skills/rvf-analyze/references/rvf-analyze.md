...
如果 finalize 发生在 Cline Kanban native task session，Stop hook 可能已经把
``$rvf-analyze <run_dir>`` 作为真实 follow-up 用户消息注入同一 task；如果发生在
Codex GUI / 非 Kanban native session，Stop hook 只会在系统提示中给出手动触发命令。
