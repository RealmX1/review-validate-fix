...
# 重构前 rvf_analyze_thread.build_analyze_command 的硬编码向量（去掉 bin 首元素），
# 作为「行为零变更」的金标准。
_CODEX_TAIL = ["--ask-for-approval", "never", "--sandbox", "workspace-write", "exec", "-"]
_CLAUDE_TAIL = ["-p", "--output-format", "stream-json", "--verbose", "--permission-mode", "acceptEdits"]
...
def test_claude_adapter_builds_stream_json_command() -> None:
    ...
    assert "acceptEdits" in cmd.argv
