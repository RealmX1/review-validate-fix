# review-validate-fix 日志系统改造计划

## 背景

当前日志系统不是一个统一系统，而是多条输出通道的组合：
...
4. 保留现有 `latest.json` 的调试价值，但把它降级为兼容指针或 summary cache。
...
flowchart TD
  ...
  subgraph After["After: unified logging"]
    ...
    AfterSystem["short systemMessage<br/>status + log path"]:::changedAfter
...
`latest.json` 保留为兼容入口，但内容应只指向最近一次 `summary.json` 和 `events.jsonl`，避免继续成为唯一状态源。
...
- 新增 `scripts/rvf_logging.py`。
- 提供 `new_run_id()`、`new_event_id()`、`write_event()`、`write_summary()`、`artifact_path()`。
- 所有写入失败都返回结构化错误，不抛出到 hook 主流程。
...
### 3. 收敛 dispatcher dev sync 日志

- `codex_stop_hook_dispatcher.py` 使用同一 `correlation_id`。
- dev sync 的每一步命令写入 `events.jsonl`，stdout/stderr 作为 artifact 保存或按大小截断。
- sync 失败时仍 fail-safe 跳过 installed hook，并在 `systemMessage` 中给出 summary path。

### 4. 连接 skill run 和 reviewer 输出

- `prepare_review_run.py` 接受可选 `--run-id` / `--log-dir`。
- `run_alternative_reviewer.py` 接受可选 `--run-id` / `--log-dir`，记录 start、activity timeout、result、contract validation 状态。
...
- 更新 `check_contracts.sh`，要求保留 `systemMessage`、`events.jsonl`、`summary.json`、`log_unavailable` 等关键契约。
