...
- 新增 `tracker_scope`：`unit_ids`、`lease_id`、`scope_hash`、`paths`、`hunks`、`source_session_id`、`takeover_from_session_id`。
- `build_review_packet.py` 优先用 tracker scope 生成 `## Tracker Scope` + path/hunk-limited `## Allocated Git Diff`。
...
2. **Slice 2-B**：`prepare_review_run.py --tracker-scope <json>`；`build_review_packet.py` 优先消费 tracker scope；`scope.contract.json` v2 字段。
