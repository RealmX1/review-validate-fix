...
| 5 | activity probe / heartbeat 矩阵 + stale-release sweeper；dispatcher 在每个 Stop event 上 refresh；TTL 默认 6h | **部分落地**：`lease_refresh` / `sweep_stale` 公共 API + reviewer-side heartbeat 已在 Slice 4 落地；Stop-hook gate 现在在 manual suppression / dry-run / allocation 前 lazy sweep stale leases；`heartbeat()` 可刷新具体 tracker lease 并记录 `rvf_state_phase` / `rvf_backend`；Cline Kanban startup prepare 会按 `tracker-scope.json.lease_ttl_seconds` 续期 tracker lease。仍待：Codex-native reviewer lease lifecycle audit、可选 Kanban task-status polling、`RVF_*_REQUEST` 显式事件语义。 |
| 6 | Stop hook gate 重构（`resolve_stop_context` / `refresh_global_diff_tracker` / `evaluate_session_gate` / `allocate_auto_review_scope` 4 函数拆分）；reason code `no_session_owned_dirty` → `no_unassigned_review_scope`；手动 fork takeover；prompt / reference 更新 | **已落地**：4 函数拆分 + reason code rename 在 Slice 3 落；手动 fork takeover（`manual_takeover` / `record_manual_rvf_run` / `_manual_suppression_scope_probe` + `evaluate_session_gate` 接线）在 Slice 5（`fbe08e6`）落；Cline Kanban startup prepare 已把 allocator 写出的 `tracker-scope.json` 接到 `prepare_review_run.py --tracker-scope`，让 packet / contract 读取 `primary_units`，并用 `tracker_scope.paths` 收窄 contract path allowlist 与 Kanban bootstrap diff；reviewer prompt / reference / generated context 已统一改读 `scope.contract.json.primary_units`，并把 session manifest 明确降级为 evidence-only |

Phase 1 的 stub（`lease_*` `NotImplementedError`、`heartbeat` no-op、`sweep_stale` 返回 `[]`）保留 API 形状，让 Phase 2–6 落地时不再破坏调用方。**截至 Phase 5 tail：`lease_acquire` / `lease_refresh` / `lease_release` / `sweep_stale` 已全部实装；`heartbeat` 已从 session/unit touch 扩展为可选 tracker lease refresh，后续只剩 Codex-native reviewer lifecycle audit 和可选 Kanban task-status polling。**
...
- **Kanban reviewer/task**：startup prepare 会在消费 tracker scope 时刷新 tracker lease，并把 phase/backend 写入 heartbeat event；更长任务若需要运行中续租，再通过 `cline_kanban_client.py` 的 `kanban task status` poll 写 heartbeat，task activity stream 与 run ledger checkpoint 同步进 events。
...
- reviewer 输出 `RVF_*_REQUEST`：当前实现保持 lease `active`，不释放 scope，直到主会话满足 request 后重试或显式取消；`paused` 保留为 schema capacity，尚未作为 workflow 状态启用。
...
| `CODEX_RVF_KANBAN_FOLLOWUP_LEASE_TTL_SECONDS` | Cline Kanban follow-up 自动 review lease TTL；用于覆盖普通 reviewer lease TTL，避免长 RVF run 在 handoff finalize 前被 stale sweep 释放 | 3600 |
...
6. **Slice 6**：reference / prompt 更新；reviewer 从 `scope.contract.json.primary_units` 取 scope；session manifest 退为 evidence-only。（已落地；阶段报告见 `docs/rvf-scope-contract-slice-6-phase-report.md`。）
   - 已实现当前 Cline Kanban 形态的 wiring：`allocate_auto_review_scope()` 把 allocator 输出存在 `ledger.tracker_scope_meta.tracker_scope_path`；`freeze_cline_kanban_dispatch_artifacts()`（Slice I 后从 `freeze_cline_kanban_startup_artifacts()` 改名）在调用 `prepare_review_run.py` 时追加 `--tracker-scope <path>`；`prepare_review_run.py` splice 进 `manifest.tracker.tracker_scope`，写出 `scope.contract.json.primary_units` / `tracker_lease_id` / `tracker_scope_hash`，让 review packet 生成 `## Tracker Scope` / `## Allocated Git Diff`，并把 `scope.contract.json.primary_files` / `fix_allowlist` 与 `worktree-bootstrap.patch` 的路径来源收敛到 `tracker_scope.paths`。同 run artifact path 作为输入时，`prepare_review_run.py` 对 `artifacts/tracker-scope.json` copy 做 same-file no-op，避免静默失败或扩大 scope。
   - RVF dispatch overhaul Slice I 已落地：tracker scope path 仍由 prep file `rvf_run.tracker_scope_path` 字段承载；`prepare_run_from_prep_file()` 直接消费该字段。语义保持不变。
...
- **RVF dispatch / fork-flow 异质性**（flow 1/2/3/4 的统一、Stop hook 拆分、UserPromptSubmit hook + prep file 机制、cline-kanban codex fork 集成、flow 2 worktree env setup、flow 3 故障态化）：见 `docs/rvf-dispatch-flow-overhaul-plan.md`。两份 plan 独立推进；当前 Cline Kanban path 已直接接入 `--tracker-scope`，未来 prep file 机制只需把路径来源从 `ledger.tracker_scope_meta` thin-refactor 到 `rvf_run.tracker_scope_path`。
